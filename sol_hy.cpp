...
double emaL = 0;      // smoothed decode population that the wave shape is chosen from
double retEma = 0;    // 0 = use the instantaneous population (old behaviour)
...
// The wave shape is re-solved from the CURRENT decode population, which changes on almost every
// frame, so mStar and dStar oscillate for the whole run.  A perfect-hindsight sweep over FIXED
// (d, m) pairs beats this adaptive choice on 20 of the 40 judge-matched instances by +3.54 mean,
// which says the oscillation itself costs more than tracking gains: each flip re-shapes waves that
// are already in flight, so the pipeline never settles into the steady state the rate model is
// solving for.  Choosing the shape from a smoothed population keeps the model's instance-awareness
// while moving it towards the fixed shape the oracle prefers.
inline void retarget() {
    int L = max(1, activeDecode);
    if (retEma > 0) {
        if (emaL <= 0) emaL = activeDecode;
        else emaL += retEma * (activeDecode - emaL);
        L = max(1, (int)(emaL + 0.5));
    }
