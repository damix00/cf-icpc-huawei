...
inline void pushXfer(bool up, double len, bool dec, int remote) {
    ...
    (up ? busyUp : busyDn) += d;
    if (up) { if (dec) dgLinkDec += d; else dgLinkPre += d; }
...
// ---- diagnostics (stderr at exit) ----
long long dgMaxPProc = 0, dgMaxPPre = 0, dgHold = 0, dgFrames = 0, dgPProcQ = 0;
double dgLinkPre = 0, dgLinkDec = 0, dgElapsed = 0, dgFirst = -1;
static void dgDump() {
    fprintf(stderr, "DIAG maxPProcQ=%lld maxPPreQ=%lld holds=%lld frames=%lld qPProcFrames=%lld"
                    " linkPre=%.6g linkDec=%.6g elapsed=%.6g
",
            dgMaxPProc, dgMaxPPre, dgHold, dgFrames, dgPProcQ, dgLinkPre, dgLinkDec, dgElapsed);
}
void schedInit(const Params& p, const Table& t) {
    static bool reg = false; if (!reg) { reg = true; atexit(dgDump); }
    dgMaxPProc = dgMaxPPre = dgHold = dgFrames = dgPProcQ = 0;
    dgLinkPre = dgLinkDec = dgElapsed = 0; dgFirst = -1;
    P = p; T = t;
    qPProc.assign(P.K, {});
    qDProc.assign(P.K, {});
    rBusy.assign(P.K, 0);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    dgFrames++;
    if (dgFirst < 0) dgFirst = t;
    dgElapsed = t - dgFirst;
    if ((long long)qPPre.size() > dgMaxPPre) dgMaxPPre = qPPre.size();
    for (int j = 0; j < P.K; j++) {
        if ((long long)qPProc[j].size() > dgMaxPProc) dgMaxPProc = qPProc[j].size();
        if (qPProc[j].size() > 1) { dgPProcQ++; break; }
    }
    retarget();
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        if (doDecode) {
            ...
            if (last && jitProc && pendingTransfers > 0) {
                ...
                else if (jitProc == 3) { hold = (blocked && rival) || (j != bestProcJ); if (hold) dgHold++; }
