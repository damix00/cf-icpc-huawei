...
int holdPreToo = 1;
// dTol makes ties for dStar go to the WIDER remote set, on the argument that extra remotes buy
// pipeline depth the rate model cannot represent.  That argument is exactly wrong when a REMOTE is
// the bottleneck: spreading a wave of m across d remotes turns one D PROC task into d of them, and
// each one costs another schedule cost S on the scarce resource.  Test #4 has now priced remote S
// three times, all negative -- idling a remote during a merge hold -48.83 (387289015), splitting a
// prefill to fit the merge window -17.44 (387290226), and CF_WAIT_P=1 -14.0 -- so on that test
// remote time is the binding resource and every avoidable S is charged in full.  bottleneck() can
// see which resource is scarce online, so drop the widening bias exactly when it is a remote.
int dTolBott = 1;
double dTolSat = 0.70;
...
void schedInit(const Params& p, const Table& t) {
    ...
    dTolBott = (int)envD("CF_DTOLB", 1);
    dTolSat = envD("CF_DTOLS", 0.70);
...
inline void retarget() {
    ...
    for (int d = 1; d <= P.K; d++) {
        ...
        // bottleneck() only asks which resource has the MOST charged work, which is a remote on
        // most instances -- gating on that alone is nearly CF_DTOL=0 globally (quiet -2.44) and
        // that is known bad.  Require the remotes to be genuinely SATURATED instead: only then is
        // the extra S per remote charged against the schedule rather than absorbed by slack.
        double dt = dTol;
        if (dTolBott && curT > t0) {
            double mr = 0;
            for (int q = 0; q < P.K; q++) mr = max(mr, busyR[q]);
            if (mr / (P.K * (curT - t0)) > dTolSat) dt = 0.0;
        }
        if (rd > bestD * (1 - dt)) { bestD = max(bestD, rd); dStar = d; }
    }
