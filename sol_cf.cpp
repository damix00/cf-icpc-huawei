...
int holdPreToo = 1;
// Chunk-to-fit.  A remote facing both a prefill and a pending decode group has only ever had two
// options, and the judge has now priced both: take the whole prefill piece and the merged D PROC
// queues behind it, or idle -- sol_hpb.cpp (387289015) idled and lost **48.83, all of it on #4**.
// But P PROC is splittable by layers, so there is a third option the code never used: run exactly
// as many layers as fit before the merge member is predicted to land.  The remote stays busy AND
// the decode group starts on time.  chunkStep already exists to size pieces; this sizes them
// against the merge deadline the D PROC hold already computes instead of a fixed target duration.
int chunkFit = 1;
double chunkFitS = 1.0;
...
void schedInit(const Params& p, const Table& t) {
    ...
    chunkFit = (int)envD("CF_CHFIT", 1);
    chunkFitS = envD("CF_CHFITS", 1.0);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        double procT1 = 1e300;
        if (!qDProc[j].empty() || chunkFit) procT1 = min(nextDecAt(true, j), nextUpAfterPre(j));
        ...
        if (doDecode) {
            ...
            // Cut the piece down to what finishes before the decode group is ready to run.  Only
            // when this remote actually has decode work coming -- otherwise the extra S per piece
            // is pure loss, which is why splitting is off by default everywhere else.
            if (chunkFit && decLoad[j] > 0 && procT1 < 1e299 && procT1 > t) {
                double perLayer = T.c[C_PPROC].at((double)lin_[i]) / (double)P.numLayers;
                if (perLayer > 1e-12) {
                    int fit = (int)((procT1 - t - chunkFitS * P.S) / perLayer);
                    if (fit >= 1) le = min(le, layersDone[i] + fit);
                }
            }
            if (le <= layersDone[i]) le = layersDone[i] + 1;
