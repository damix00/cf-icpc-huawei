...
// D PRE fixes a wave's width for a whole round trip, but it has no hold of its own -- it is only
// ever delayed as a side effect of holding D POST, so with an empty D POST queue a wave fires at
// whatever width happens to be ready.  An open-ended hold here is measurably wrong (it delays
// decode far too often), but offline hill climbing over per-frame decisions keeps picking single
// frames to skip: the useful delay is one event long, just enough for a member that is already
// in flight to land.  So bound it by events, not by a time budget.
int dpreSkipMax = 0;      // consecutive frames a short D PRE wave may wait (0 = never)
int dpreSkips = 0;
int holdPreToo = 1;
...
void schedInit(const Params& p, const Table& t) {
    ...
    dpreSkipMax = (int)envD("CF_DSKIP", 0);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    // Wait one event for a wave that is short of its target, at most dpreSkipMax times in a row.
    bool holdDPre = false;
    if (dpreSkipMax > 0 && inFlight && !eligDPre.empty() && (int)eligDPre.size() < mStar
        && activeDecode > (int)eligDPre.size() && dpreSkips < dpreSkipMax
        && batchingHelpsBottleneck(mStar)) holdDPre = true;

    if (holdDPre) dpreSkips++; else if (!eligDPre.empty()) dpreSkips = 0;
    ...
    if (!eBusy) {
        ...
        for (int p = 0; ord[p] && choice < 0; p++) {
            ...
            if ((c == 0 && !qDPost.empty() && !holdPost) || (c == 1 && !qPPost.empty()) ||
                (c == 2 && !eligDPre.empty() && !holdDPre && !(holdPost && holdPreToo)) ||
