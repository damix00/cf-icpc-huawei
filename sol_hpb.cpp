...
int holdPreToo = 1;
// The remote side of the same mistake.  When the D PROC merge hold fires, doDecode is cleared and
// control falls straight through to this remote's P PROC queue -- so the remote starts a prefill
// chunk at exactly the moment it was supposed to be sitting free waiting for the merge member, and
// the merged D PROC then queues behind it.  Unblocking the local computer's D PRE during a D POST
// hold was submitted as 387282957 and cost -7.9 on the judge (#4 -4.3, #8 -2.7, #6 -0.8), which is
// this same effect measured: whatever the hold lets a resource do instead, it pays for.
int holdProcBlock = 1;
...
void schedInit(const Params& p, const Table& t) {
    ...
    holdProcBlock = (int)envD("CF_HOLDPROCB", 1);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        if (doDecode && inFlight && (int)qDProc[j].size() < mStarR && batchingHelpsBottleneck(mStarR)) {
            ...
            if (t1 - t <= budget && t - holdProcSince[j] <= holdCap * budget) { doDecode = false; procHeld = true; }
        ...
        } else if (!qPProc[j].empty() && !(procHeld && holdProcBlock)) {
