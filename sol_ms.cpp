...
// The D POST merge hold stops at mStar, the group size the analytic rate model likes.  That model
// prices throughput only; it cannot see that stretching a token gap is FREE while the realised
// TPOT still sits under SLO2 (the excess is clamped at zero).  So where TPOT has measured slack,
// let the hold target a larger group than the model asked for -- the extra merge saves a whole S
// per member on the local computer, and costs nothing the scorer can see.  Gated on the same
// measurement the P PRE hold uses, so a TPOT-critical instance is excluded outright.
double mSlack = 0.0;       // multiplier on mStar for the D POST hold; 0/1 disables
double mSlackM = 0.5;      // ... only while tpotNow() < mSlackM * SLO2
...
void schedInit(const Params& p, const Table& t) {
    ...
    mSlack = envD("CF_MSLACK", 2.0);
    mSlackM = envD("CF_MSLACKM", 0.5);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    bool holdPost = false;
    int mPost = mStar;
    if (mSlack > 1 && P.SLO2 > 0 && gapCnt > 0 && tpotNow() < mSlackM * P.SLO2)
        mPost = max(mStar, min(max(1, activeDecode), (int)(mStar * mSlack)));
    if (inFlight && !qDPost.empty() && (int)qDPost.size() < mPost && batchingHelpsBottleneck(mPost)) {
