...
// The swap buys throughput and pays in TPOT.  excess_tpot is clamped at zero, so while the
// realised gap sits under SLO2 that payment is free and the swap should always fire; once the gap
// is over SLO2 every extra millisecond is charged against the waiting component.  Both quantities
// are known online, so the second half of the trade no longer has to be guessed either.
double swapTpot = 1.0;      // 0 disables the TPOT half of the gate
double swapMin = 0.05;
int swapWarm = 8;
