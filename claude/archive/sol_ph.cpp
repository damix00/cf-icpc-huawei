...
int jitPre = 0, jitProc = 1, jitMode = 1;
double jitSlack = 1.0;
...
int deferFirst = 1;          // 0 off, 1 while a prefill transfer is on a link, 2 while any prefill is unfinished
...
void schedInit(const Params& p, const Table& t) {
    ...
    jitProc = (int)envD("CF_JITR", 1);
    jitSlack = envD("CF_JITS", 1.0);
    jitMode = (int)envD("CF_JITM", 1);
    deferFirst = (int)envD("CF_DEFER", 1);
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        if (doDecode) {
            ...
            if (last && jitProc && pendingTransfers > 0) {
                ...
                if (lim > t + jitSlack * (P.S + dur)) hold = true;           // no rush: link busy
                else if (j != bestProcJ) hold = true;                       // shorter one first
            }
            if (!hold) {
