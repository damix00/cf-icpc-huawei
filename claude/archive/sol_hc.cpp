...
// ---- offline hill-climbing harness (diagnostic) --------------------------------------------
// CF_HCLIST names a file of frame indices at which the local computer is forced to idle.  Hill
// climbing over that set answers "how much is a perfect per-frame decision worth?" without
// having to build the policy that would find it online.
set<long long> hcIdle;
long long hcFrame = 0;
...
void schedInit(const Params& p, const Table& t) {
    ...
    tokensOut = 0;
    hcIdle.clear(); hcFrame = 0;
    if (const char* fn = getenv("CF_HCLIST")) {
        FILE* fp = fopen(fn, "r");
        if (fp) { long long v; while (fscanf(fp, "%lld", &v) == 1) hcIdle.insert(v); fclose(fp); }
    }
}
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    bool hcSkip = inFlight && hcIdle.count(myFrame);
    if (hcSkip && !eBusy && getenv("CF_HCTRACE")) {
        int c = -1;
        const char* ordT = prefillUrgent ? ordAdmit : ordDecode;
        for (int p = 0; ordT[p] && c < 0; p++) {
            int cc = ordT[p] - '0';
            if ((cc == 0 && !qDPost.empty() && !holdPost) || (cc == 1 && !qPPost.empty()) ||
                (cc == 2 && !eligDPre.empty() && !(holdPost && holdPreToo)) ||
                (cc == 3 && !qPPre.empty())) c = cc;
        }
        static const char* nmT[4] = {"DPOST", "PPOST", "DPRE", "PPRE"};
        fprintf(stderr, "IDLE f=%lld t=%.3f skip=%s qDPre=%d qDPost=%d qPPre=%d qPPost=%d "
                        "mStar=%d dStar=%d L=%d upBusy=%.3f dnBusy=%.3f\n",
                myFrame, t, c < 0 ? "none" : nmT[c], (int)qDPre.size(), (int)qDPost.size(),
                (int)qPPre.size(), (int)qPPost.size(), mStar, dStar, activeDecode,
