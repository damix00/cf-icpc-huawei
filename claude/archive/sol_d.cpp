...
double jitSlack = 1.0;
// Delayed-SPT (Hoogeveen-Vestjens): on a single non-preemptive server with unknown release dates,
// never start a job before time proportional to its own length.  Here the server is the uplink and
// the job length is the prefill payload, so a 4096-token request waits a while before it is allowed
// to seize the link -- long enough for a 6-token request that arrives meanwhile to go first.  The
// first request of a run is exactly the case the plain shortest-first rule cannot help with,
// because when it arrives it is the only candidate.  Gated to ramp-up like the reordering hold.
double dsptW = 0.0;
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    if (jitPre && activeDecode <= jitL && !qPPre.empty() && pendingTransfers > 0) {
        ...
        else if (dsptW > 0 && t < arrT[i] + dsptW * (P.lat + uPerToken * lin_[i])) holdPPre = true;
    }
