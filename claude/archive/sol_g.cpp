...
// Gate variants for the P PRE reordering hold (CF_JITG):
//   0  decode population <= jitL                (shipped)
//   1  no token produced yet
//   2  population gate, or a pool worth sorting (>= jitPool waiting)
//   3  displaced decode traffic small next to the payload being reordered
int jitG = 0, jitPool = 3;
double jitRatio = 0.1;
double jitSlack = 1.0;
...
void schedInit(const Params& p, const Table& t) {
    ...
    jitG = (int)envD("CF_JITG", 0);
    jitPool = (int)envD("CF_JITPOOL", 3);
    jitRatio = envD("CF_JITRATIO", 0.1);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    if (jitPre && !qPPre.empty() && pendingTransfers > 0) {
        int i = peekPPre();
        bool gate;
        if (jitG == 1)      gate = (tokensOut == 0);
        else if (jitG == 2) gate = (activeDecode <= jitL) || ((int)qPPre.size() >= jitPool);
        else if (jitG == 3) gate = activeDecode * (P.lat + uPerToken) < jitRatio * uPerToken * lin_[i];
        else                gate = (activeDecode <= jitL);
        double lim = jitMode ? lastPreFin(true) : upFreeAt;
        if (gate && lim > t + jitSlack * (P.S + T.c[C_PPRE].at(lin_[i]))) holdPPre = true;
    }
