...
double eBottleW = 1.0;
double coLoc = 0.0;   // bonus for placing onto a remote that is already decoding
...
inline int pickRemote() {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        // A request placed on a remote that is already decoding joins that remote's existing
        // D PROC groups instead of forcing new ones: it costs the marginal group-size increment
        // rather than a whole schedule cost plus a one-member D PROC, every round for the rest of
        // its life.  Nothing in a pure delay estimate sees that, so price it explicitly.
        if (coLoc > 0 && decLoad[j] > 0) cost -= coLoc * (P.S + T.c[C_DPROC].at(1.0));
