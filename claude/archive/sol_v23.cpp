...
double tieEps = 0.0;            // ties within tieEps*cMin are broken by load, then by recency
vector<double> lastUsed;   // hold-budget ceiling while the local computer is the bottleneck
...
inline int pickRemote() {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        // Ties used to resolve to the lowest index, which quietly concentrates every equal-cost
        // placement (notably every idle remote at cost 0) onto remote 0.  Break ties by how many
        // requests the remote already carries, then by which was used least recently.
        double eps = tieEps * cMin;
        bool better = cost < bestCost - eps;
        if (!better && cost < bestCost + eps && best >= 0)
