...
int holdPreToo = 1;
// Prefill-vs-decode arbitration on the DOWNLINK.  The downlink is a single-server FIFO carrying two
// completely different sizes of work: a prefill result is lat + u * L_in (up to 4096 tokens' worth)
// while a decode result is lat + u * 1.  A decode result that queues behind one big prefill result
// stalls that request's whole serial token loop for the length of the prefill transfer, and the
// UPLINK then idles because nothing new can be released into it.  judge/j_57 is the clean case:
// both links are the bottleneck at 72.4 % and the uplink idles 1.5e6 ms in a single block at the
// tail (UPd[4.269e6..5.77e6]) with every computer empty -- the decode loop is simply not turning.
// jitProc already reorders the last P PROC chunk against SHORTER PREFILLS; this extends the same
// look-ahead across the remote stage to decode work, which nextDownAfterProc() already predicts.
// Bounded by jitDecCap * SLO1 of accumulated wait so a prefill can never be starved outright.
int jitDec = 1;
double jitDecR = 1.0, jitDecCap = 0.5;
vector<double> jitDecSince;
...
void schedInit(const Params& p, const Table& t) {
    ...
    jitDec = (int)envD("CF_JITD", 1);
    jitDecR = envD("CF_JITDR", 1.0);
    jitDecCap = envD("CF_JITDCAP", 0.5);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        if (doDecode) {
            ...
            // ... and the same look-ahead against decode results still on a remote.
            if (last && jitDec && !hold && pendingTransfers > 0 && activeDecode > 0) {
                double myDown = P.lat + uPerToken * (double)lin_[i];
                double dr = nextDownAfterProc();
                if (jitDecSince[i] < 0) jitDecSince[i] = t;
                bool capped = (P.SLO1 > 0) && (t - jitDecSince[i] > jitDecCap * P.SLO1);
                // mode 1: only a D PROC actually running (needs the remote stage to be slow).
                // mode 2: any live decode at all -- on a transfer-dominated instance a decode result
                // is generated every round trip, so "one will land during my transfer" is certain
                // whenever myDown is long compared with the decode round trip itself.
                double decRound = 2 * (P.lat + uPerToken) + 3 * P.S;
                bool rivalDec = (jitDec >= 2) ? (myDown > jitDecR * decRound)
                                              : (dr < 1e299 && dr < t + jitDecR * myDown);
                if (!capped && rivalDec) hold = true;
            }
            if (!hold) jitDecSince[i] = -1;
            if (!hold) {
