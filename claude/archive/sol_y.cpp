...
int jitPre = 0, jitProc = 3, jitMode = 1;
double jitSlack = 1.0;
// Idle a computer rather than block an imminent prefill, but only while the idle is short
// relative to the block it avoids: yieldE/yieldR are that ratio (0 disables).
double yieldE = 1.0, yieldR = 1.0;
...
// ---- anticipate prefill, do not block it -------------------------------------------------
// TDR's clock stops at P POST, and P POST needs the local computer.  A decode task started at t
// holds E until t+S+dur, so a prefill-down transfer landing inside that window has its P POST
// pushed back by the remainder -- pure TDR loss that the ordering rule cannot prevent, because
// by the time the transfer lands E is already committed.  The link mirror knows exactly when it
// will land, so the block is foreseeable and therefore avoidable: idle E for the short remainder
// instead.  Same argument on a remote, where a running D PROC delays the P PROC of a prefill
// whose uplink transfer is about to complete.
inline double nextPPostAt() {
    double best = 1e300;
    for (auto& p : downQ) if (!p.dec) { best = p.fin; break; }
    for (int j = 0; j < P.K; j++)
        if (rBusy[j] && rRec[j].step == ST_PPROC && rRec[j].le >= P.numLayers && !rRec[j].ids.empty())
            best = min(best, max(rFreeAt[j], downFreeAt) + P.lat + uPerToken * (double)lin_[rRec[j].ids[0]]);
    return best;
}
inline double nextPProcAt(int j) {
    double best = 1e300;
    for (auto& p : upQ) if (!p.dec && p.remote == j) { best = p.fin; break; }
    if (eBusy && eRec.step == ST_PPRE && !eRec.ids.empty() && rem[eRec.ids[0]] == j)
        best = min(best, max(eFreeAt, upFreeAt) + P.lat + uPerToken * (double)lin_[eRec.ids[0]]);
    return best;
}
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    bool holdPPre = false;
    if (jitPre && !qPPre.empty() && pendingTransfers > 0) {
        int i = peekPPre();
    ...
    if (!eBusy) {
        ...
        // would this decode task block a P POST that is about to become ready?
        if (yieldE > 0 && (choice == 0 || choice == 2) && qPPost.empty() && inFlight) {
            double dur = (choice == 0) ? T.c[C_DPOST].at((double)qDPost.size())
                                       : T.c[C_DPRE].at((double)eligDPre.size());
            double fin = t + P.S + dur, tPP = nextPPostAt();
            if (tPP < fin && tPP - t <= yieldE * (fin - tPP)) choice = -1;
        }
        if (choice == 0) {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        if (doDecode && yieldR > 0 && qPProc[j].empty() && inFlight) {
            double dur = T.c[C_DPROC].at((double)qDProc[j].size());
            double fin = t + P.S + dur, tPP = nextPProcAt(j);
            if (tPP < fin && tPP - t <= yieldR * (fin - tPP)) doDecode = false;
        }
        if (doDecode) {
