...
int holdPreToo = 1;
double openThresh = 16.0, openLink = 1.0;
// D PRE vs D POST: D PRE hands work DOWNSTREAM (uplink, then a remote), D POST only refills a queue
// E already owns, so running D PRE first keeps the pipeline fed.  Worth +3.9 on the judge across 21
// of its 22 tests -- and -42.1 on the one test that sits at tpComp ~= 0.001 with the waiting
// component exactly 1.0.  There, throughput is simply not winnable, so the small TPOT slip the swap
// costs buys nothing at all.  The score function tells us which case we are in: only spend waiting
// on throughput once we can see throughput actually being won.
double swapMin = 0.05;
int swapWarm = 8;
long long tokensOut = 0;   // when a fresh remote is worth opening
double cMin = 1.0;                         // minimum decode round trip: this instance's time scale
double arrPProc = 0, arrPXfer = 0;         // committed-at-arrival prefill work, compute vs link
...
inline int pickRemote() {
    ...
    // dStar answers "how many remotes should one decode WAVE span?" -- a wave spanning d remotes
    // emits d uplink transfers, so on a high-latency instance the model rightly returns 1.  Using
    // that same number as a hard cap on PLACEMENT is a different claim, and a wrong one: a wave
    // whose members all live on remote j still emits exactly one transfer, so refusing an idle
    // remote buys nothing.  Left coupled, every request lands on remote 0 while the others idle for
    // the entire run (h_4_15: rem=0.945,0,0,0,0 across 141 s against a work floor of 26.6 s).
    // Open a fresh remote once the prefill already committed to the best open one exceeds a decode
    // round trip -- but only when this instance is short of REMOTE time rather than LINK time.
    // arrPProc/arrPXfer are totalled at arrival from Lin alone, so unlike the busy* accumulators
    // they do not depend on the placement they are being used to decide (gating on those makes the
    // rule chase its own tail: spread, link looks busy, stop spreading, re-concentrate).
    ...
    bool allowNew = (active < dStar) || (remoteBound && bestOpen > openThresh * cMin);
    int best = -1;
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    // Is throughput actually being won on this instance?  tp is tokens over elapsed and both terms
    // are known, so the realised share of the (tpBase, tpUB) span is directly observable.  It
    // converges from below during ramp-up, which is the safe direction: the swap stays off until
    // there is real throughput to protect.
