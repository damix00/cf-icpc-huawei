...
// Delayed-SPT (Hoogeveen-Vestjens): on a non-preemptive server with unknown release dates, never
// start a job before a time proportional to its own length.  The uplink is the server and the
// prefill payload is the length, so a 4096-token request waits before it may seize the link --
// long enough for a 6-token request arriving meanwhile to overtake it.  This is the one case the
// shortest-first pop cannot help with: the first request of a run is the only candidate there is.
double dsptW = 0.0;
