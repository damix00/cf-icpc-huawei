...
// Which OTHER local steps a D POST merge hold also blocks, as a bit mask: 1 = D PRE, 2 = P POST,
// 4 = P PRE.  Unblocking D PRE (mask 0) was submitted as 387282957 and cost -7.9 on the judge,
// spread over #4 -4.3, #8 -2.7, #6 -0.8, #13, #10 -- so running any other task during the hold
// makes E busy exactly when the merge member lands, and the merged D POST then queues behind it.
// The hold's whole point is to have E free at that moment.  P POST and P PRE were never blocked,
// so on those five tests E has been spending every hold on prefill work for the same reason.
int holdPreToo = 1;
...
void schedInit(const Params& p, const Table& t) {
    ...
    holdPreToo = (int)envD("CF_HOLDPRE", 3);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    if (!eBusy) {
        ...
        for (int p = 0; ord[p] && choice < 0; p++) {
            int c = ord[p] - '0';
            bool blocked = holdPost && (holdPreToo & (c == 2 ? 1 : c == 1 ? 2 : c == 3 ? 4 : 0));
            if ((c == 0 && !qDPost.empty() && !holdPost) || (c == 1 && !qPPost.empty() && !blocked) ||
                (c == 2 && !eligDPre.empty() && !blocked) ||
                (c == 3 && !qPPre.empty() && !holdPPre && !blocked)) choice = c;
        }
        if (choice == 0) {
