...
int holdPreToo = 1;
// Prefill/decode arbitration ON A REMOTE.  poolTarget defaults to 1e9, so prefillUrgent is ALWAYS
// true, so this line makes a remote defer decode to prefill unconditionally whenever it holds any
// prefill work.  That arbitration was never judge-tested; the one probe that touched it is
// sol_hpb.cpp (387289015), which made a remote idle rather than take prefill during a D PROC merge
// hold and lost **48.83 points, every one of them on test #4** -- more than any other single-test
// move recorded here except CF_WAIT_P=1's -70.8 on #5.  So #4's schedule turns on exactly this
// decision, and the only direction never measured is the other one.  quiet reads -0.09 for it,
// which after CF_WAIT_P (local suites flat, judge +36.7) is not evidence of anything.
int remPreFirst = 1;
...
void schedInit(const Params& p, const Table& t) {
    ...
    remPreFirst = (int)envD("CF_RPRE", 0);
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        if (doDecode && remPreFirst && prefillUrgent && !qPProc[j].empty()) doDecode = false;
        if (doDecode) {
