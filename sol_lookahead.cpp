...
double waitPost = 8.0, waitProc = 4.0, waitPre = 0.0, holdCap = 4.0, warmUp = 100.0, dTol = 0.04;
int holdPreToo = 1;
...
void schedInit(const Params& p, const Table& t) {
    ...
    // Complete completed decode work first, then admit a new request.  This keeps the local
    // computer work-conserving while feeding the pipeline before it spends a slot on an already
    // prefetched request; it beats the weight-only P PRE/P POST split on all three held-out suites.
    ordAdmit = envS("CF_ORD_A", "0312");
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    // A D PRE can be delayed briefly when a decode result is already travelling back to E.
    // That preserves the safety invariant (the transfer guarantees another frame) while giving
    // the returned requests a chance to join this uplink wave and save one local task/latency.
    bool holdPre = false;
    if (waitPre > 0 && inFlight && !qDPre.empty() && (int)qDPre.size() < mStar &&
        batchingHelpsBottleneck(mStar)) {
        double t1 = nextDecAt(false, -1);
        double budget = waitPre * mergeSaving(T.c[C_DPRE], (double)qDPre.size(), P.S);
    ...
    if (!eBusy) {
        ...
        for (int p = 0; ord[p] && choice < 0; p++) {
            ...
            if ((c == 0 && !qDPost.empty() && !holdPost) || (c == 1 && !qPPost.empty()) ||
                (c == 2 && !qDPre.empty() && !(holdPost && holdPreToo) && !holdPre) ||
