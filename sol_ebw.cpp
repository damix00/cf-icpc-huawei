...
void schedInit(const Params& p, const Table& t) {
    ...
    // `eBottleW` clamps the merge-hold budget to ONE merge saving whenever the local computer is
    // the bottleneck -- a much more active constraint than `waitPost` itself, which stops binding
    // above 16 on every local suite.  The judge has now twice said that longer holds are better on
    // this axis (NOTES 17), so releasing the clamp is the coherent next step: at 16 it equals
    // `waitPost` and is inert, leaving one rule instead of two.
    eBottleW = envD("CF_EBW", 16.0);
