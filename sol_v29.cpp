...
double eBottleW = 1.0;
double openThresh = 0.0, openLink = 1.0;
double cMin = 1.0;
double arrPProc = 0, arrPXfer = 0;
...
inline int pickRemote() {
    ...
    // Open a fresh remote only when this instance is short of REMOTE time rather than LINK time --
    // widening the decode set costs one more transfer latency per wave in each direction, which is
    // exactly what a throughput-weighted, latency-bound instance cannot afford.
    bool remoteBound = arrPProc / P.K > openLink * arrPXfer;
    bool allowNew = (active < dStar) || (openThresh > 0 && remoteBound && bestOpen > openThresh * cMin);
    for (int j = 0; j < P.K; j++) {
