...
int mAll = 1;         // 1 = target the whole live decode population, not the rate model's choice
...
inline void retarget() {
    ...
    // The merge holds only ever fire while the group is still short of `mStar`, so `mStar` -- not
    // the budget -- is what ultimately caps a decode group.  The rate model's `L/C` term drives it
    // down, because it assumes waves pipeline perfectly; when a remote serialises D PROC they do
    // not, and the model then under-sizes every wave.  The judge has twice paid for longer holds
    // (NOTES 17), so this probe asks for the opposite extreme: target the whole live population.
    if (mAll) mStar = max(mStar, L);
