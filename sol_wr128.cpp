...
void schedInit(const Params& p, const Table& t) {
    ...
    // Same axis as CF_WAIT_P, one stage over.  waitPost 8 -> 32 was worth +36.7 on the judge while
    // every local suite reported 16, 32 and 64 as byte-identical, because the budget stops binding
    // locally long before it stops binding on the judge -- and 4 -> 14 here was measured at exactly
    // 0.000, which is what "still below the judge's binding point" looks like, not what "inert"
    // looks like.  quiet reads 14, 32 and 128 identical to three decimals while 1 does move it,
    // so the knob is live and the local instrument is blind above the plateau, exactly as before.
    waitProc = envD("CF_WAIT_R", 128.0);
