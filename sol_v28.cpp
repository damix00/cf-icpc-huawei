...
double eBottleW = 1.0;
double qdW = 1.0;     // weight on a remote's QUEUED decode work when placing (0 = correct model)
...
inline int pickRemote() {
    ...
    for (int j = 0; j < P.K; j++) {
        ...
        // Only the task the remote is RUNNING delays our P PROC: prefill preempts decode when the
        // remote frees, so a queued D PROC group never gets in front of us.  Counting it (qdW > 0)
        // is the naive version of this estimate.
        if (remBusyW > 0) cost += remBusyW * (max(0.0, rFreeAt[j] - curT)
                                  + qdW * (qDProc[j].empty() ? 0.0 : P.S + T.c[C_DPROC].at((double)qDProc[j].size())));
