...
struct Xf { double fin; char dec; int remote; int rid; };  // remote: which D PROC queue an UP feeds
deque<Xf> upQ, downQ;

inline void pushXfer(bool up, double len, bool dec, int remote, int rid = -1) {
    ...
    freeAt = start + d;
    (up ? upQ : downQ).push_back({freeAt, (char)dec, remote, rid});
}
...
double jitSlack = 1.0;
double rivalW = 1.0;      // how far ahead a shorter prefill may be and still be worth waiting for
...
// Earliest moment a strictly shorter prefill could take the downlink slot instead of ours.
// Mean TDR is a mean completion time on that FIFO, so when a shorter job can reach the link
// before our own transfer would have finished, letting it go first is the textbook SPT swap --
// it strictly lowers the sum of completion times.  Requests already sitting on a remote are
// ready now; requests still in the uplink queue are ready when their transfer lands.
inline double rivalReady(int myLin) {
    double best = 1e300;
    for (int j = 0; j < P.K; j++)
        for (int i : qPProc[j])
            if (lin_[i] < myLin) { best = curT; break; }
    if (best <= curT) return best;
    for (auto& p : upQ)
        if (!p.dec && p.rid >= 0 && lin_[p.rid] < myLin) best = min(best, p.fin);
    return best;
}
...
inline void onTaskDone(int server) {
    ...
    switch (rec.step) {
        case ST_PPRE: {
            ...
            st[i] = R_WAIT_PRE_UP;
            pendingTransfers++;
            pushXfer(true, lin_[i], false, rem[i], i);
            break;
        }
        case ST_PPROC: {
            int i = rec.ids[0];
            ...
            layersDone[i] = rec.le;
            if (rec.le >= P.numLayers) { st[i] = R_WAIT_PRE_DOWN; pendingTransfers++; pushXfer(false, lin_[i], false, rem[i], i); }
        ...
        if (doDecode) {
            ...
            if (last && jitProc && pendingTransfers > 0) {
                ...
                if (jitProc == 1) hold = blocked || j != bestProcJ;
                    ...
                    // ramp-up only: before anything is decoding, reordering the downlink is free
                    double mine = t + P.S + dur + P.lat + uPerToken * (double)lin_[i];
                    bool soon = rival && activeDecode == 0 && rivalReady(lin_[i]) <= t + rivalW * (mine - t);
                    hold = (blocked && rival) || (j != bestProcJ) || soon;
                }
                else if (jitProc >= 5) {
                    // SPT swap: yield the slot whenever a shorter prefill can reach the link
                    // before our transfer would have cleared it.
                    double mine = t + P.S + dur + P.lat + uPerToken * (double)lin_[i];
                    bool soon = rival && rivalReady(lin_[i]) <= t + rivalW * (mine - t);
                    hold = (blocked && rival) || (j != bestProcJ) || soon;
                }
            }
            if (!hold) {
