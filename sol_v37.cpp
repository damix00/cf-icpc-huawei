...
void schedInit(const Params& p, const Table& t) {
    ...
    // Order digits: 0 = D POST, 1 = P POST, 2 = D PRE, 3 = P PRE.  Running D POST first costs the
    // prefill steps one group time but banks tokens already sitting on the local computer, and the
    // P PRE that follows keeps admission ahead of P POST.  Measured on every suite at once this is
    // the best of all 24 orders on the current base -- the old "P POST first" default predates the
    // P PRE hold, which now regulates admission on its own.
    ordAdmit = envS("CF_ORD_A", P.wTp >= envD("CF_ORDW", 0.75) ? "3102" : "0312");
