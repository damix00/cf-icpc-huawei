...
// D PRE had no hold of its own: it was only ever delayed as a side effect of holding D POST, so
// whenever the D POST queue happened to be empty a wave fired at whatever width was ready --
// often one request when the model wanted several.  D PRE is the step that fixes a wave's width
// for the whole round trip (its completion queues one uplink transfer per remote in the group),
// so merging there saves a local task AND a transfer latency, strictly more than merging D POST.
double waitPre = 8.0;
int dTgt = 0;             // 0 = model target, 1 = whole live population, 2 = the larger
double holdPreSince = -1;
int holdPreToo = 1;
...
int jitL = 0;                // hold P PRE only while at most this many requests are decoding
double jitSlack = 1.0;
...
void schedInit(const Params& p, const Table& t) {
    ...
    holdPostSince = -1; holdPreSince = -1;
    ...
    waitPre = envD("CF_WAIT_D", 8.0);
    dTgt = (int)envD("CF_DTGT", 0);
...
// Earliest moment a request could join qDPre.  Only two things put one there: a P POST (a newly
// prefilled request) or a D POST (a request that just produced a token), and both run on E, so
// the answer is always "when E next finishes one of those" -- or when the transfer that will
// feed one lands.
inline double nextDPreAt() {
    double best = 1e300;
    if (eBusy && (eRec.step == ST_DPOST || eRec.step == ST_PPOST)) best = min(best, eFreeAt);
    if (!qDPost.empty()) best = min(best, curT + P.S + T.c[C_DPOST].at((double)qDPost.size()));
    if (!qPPost.empty()) best = min(best, curT + P.S + T.c[C_PPOST].at((double)lin_[qPPost.front()]));
    double dn = 1e300;
    for (auto& p : downQ) { dn = p.fin; break; }
    dn = min(dn, nextDownAfterProc());
    if (dn < 1e299) best = min(best, dn + P.S + T.c[C_DPOST].at(1.0));
    return best;
}
...
void schedFrame(double t, const Frame& f, Response& out) {
    ...
    bool holdPPre = false;
    if (jitPre && activeDecode <= jitL && !qPPre.empty() && pendingTransfers > 0) {
        int i = peekPPre();
    ...
    // Wait for one more member exactly while the wave is short of its target and the next member
    // is closer than the merge is worth -- the same trade already made for D POST.
    bool holdPre = false;
    int dWant = dTgt == 1 ? activeDecode : dTgt == 2 ? max(mStar, activeDecode) : mStar;
    if (inFlight && !eligDPre.empty() && (int)eligDPre.size() < dWant && batchingHelpsBottleneck(dWant)) {
        double t1 = nextDPreAt();
        double budget = waitPre * mergeSaving(T.c[C_DPRE], (double)eligDPre.size(), P.S);
        if (holdPreSince < 0) holdPreSince = t;
        if (t1 - t <= budget && t - holdPreSince <= holdCap * budget) holdPre = true;
    }
    if (!holdPre) holdPreSince = -1;
    ...
    if (!eBusy) {
        ...
        for (int p = 0; ord[p] && choice < 0; p++) {
            ...
            if ((c == 0 && !qDPost.empty() && !holdPost) || (c == 1 && !qPPost.empty()) ||
                (c == 2 && !eligDPre.empty() && !holdPre && !(holdPost && holdPreToo)) ||
