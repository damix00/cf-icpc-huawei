...
void schedInit(const Params& p, const Table& t) {
    ...
    // The cap on TOTAL hold time, in budgets.  With eBottleW measured inert and waitPost 16 == 32
    // on every suite, this and mStar are the only things still capping a decode group (NOTES 17b).
    holdCap = envD("CF_HOLDCAP", 16.0);
